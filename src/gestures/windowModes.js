async function enterWindowMode(ctx, mode, refPt, hand) {
    ...
    state.gcr.acquire(mode === 'move' ? 'windowMove' : 'windowResize', ctx.extCount(hand));
  ...
  function exitWindowMode(ctx) {
    ...
    state.windowMode = 'none'; state.windowRefPt = null;
  }
  
  async function tickWindowMode(ctx, currPt, hand) {
    const { state, CFG } = ctx;
    if (state.windowMode === 'none') return;
    const t = ctx.now(); if (t - state.lastWindowTick < CFG.windowTickMs) return;
    ...
    if (state.windowMode === 'move') {
      ...
      if (Math.abs(dw) > 1 || Math.abs(dh) > 1) {
        ...
        state.resizeBaseline.roll = roll; state.resizeBaseline.pitch = pitch;
      }
    }
  }
